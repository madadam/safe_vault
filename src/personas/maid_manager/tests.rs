...
fn account_replication_during_churn() {
    ...
    assert_eq!(message.src, Authority::ManagedNode(unwrap!(old_node.name())));
    assert_eq!(message.dst, client_manager);
    let refresh = assert_match!(message.request, Request::Refresh(payload, _) => payload);

    // After new node receives the refresh from at least QUORUM nodes, it gets the account too.
    for _ in 0..(QUORUM - 1) {
        unwrap!(new_mm.handle_refresh(&mut new_node, &refresh, msg_id));
    }
...
fn refresh_data_ops_count() {
    ...
    let (account_name, ops_count) =
        assert_match!(refresh, Refresh::UpdateData { name, ops_count } => (name, ops_count));
    assert_eq!(ops_count, balance_0.mutations_done + 1);

    // Simulate receiving the refresh, but not everyone sends the same value.
    for value in &[0, 1, 2, 3, 4, 5, 6, 7] {
        let refresh = Refresh::UpdateData {
            name: account_name,
            ops_count: balance_0.mutations_done + value,
        };
        let refresh = unwrap!(serialise(&refresh));

        unwrap!(mm.handle_refresh(&mut node, &refresh, msg_id));
    }

    // The resulting balance should be the median of the first QUORUM values.
    let balance_1 = unwrap!(get_account_info(&mut node, &mut mm, client, client_manager));
    assert_eq!(balance_1.mutations_done, balance_0.mutations_done + 2);
}
...
fn simulate_refresh(node: &mut RoutingNode,
    ...
    for _ in 0..count {
        unwrap!(mm.handle_refresh(node, &refresh, msg_id));
    }
}
